...
# I could never get this to complete...
so I make it into a sub-function
using Random
using LinearAlgebra
plotlyjs()
...
## Show various fits to the subsets
...
for i=1:k
  subset = randperm(size(A,1))[1:nsamp] # choose 25 of 200 samples.
  ab = A[subset,:]\b[subset] # solve the least squares problem
  plot!(x, x*ab[1] .+ ab[2], label="",color=5,alpha=0.5)
end
#title!(@sprintf("height ≈ %.2f weight + %.2f    ",ab[1],ab[2]))
xlabel!("Weight (kg)")
ylabel!("Height (cm)")
savefig("weight-height-multi-fit.pdf")
